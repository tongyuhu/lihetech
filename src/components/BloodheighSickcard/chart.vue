<template>
  <div>
    ...
    <el-card>
      ...
      <el-row>
        <el-col :span="6">
          <h3>{{xueyaqushiTotal.totalTest}}次</h3>
          <p>总测量次数</p>
        </el-col>
        <el-col :span="6">
          <h3>{{xueyaqushiTotal.normal}}次</h3>
          <p>正常偏高次数</p>
          <p>121/81~139/89</p>
        </el-col>
        <el-col :span="6">
          <h3>{{xueyaqushiTotal.heigh}}次</h3>
          <p>高血压次数</p>
          <p>140/90~179/109</p>
        </el-col>
        <el-col :span="6">
          <h3>{{xueyaqushiTotal.danger}}次</h3>
          <p>危险次数</p>
          <p>>179/109</p>
        </el-col>
      </el-row>
      ...
      <!-- 标签 -->
      <el-row>
        <el-row :gutter="5" class="tag">
          <el-col :span="4" class="tag-btn-box">
            <el-button type="text" size="medium" class="tag-btn">日常</el-button>
          </el-col>
          <el-col :span="4" class="tag-btn-box">
            <el-button type="text" size="medium" class="tag-btn">入厕后</el-button>
          </el-col>
          <el-col :span="4" class="tag-btn-box">
            <el-button type="text" size="medium" class="tag-btn">抽烟后</el-button>
          </el-col>
          <el-col :span="4" class="tag-btn-box">
            <el-button type="text" size="medium" class="tag-btn">喝酒后</el-button>
          </el-col>
          <el-col :span="4" class="tag-btn-box">
            <el-button type="text" size="medium" class="tag-btn">情绪波动</el-button>
          </el-col>
          <el-col :span="4" class="tag-btn-box">
            <el-button type="text" size="medium" class="tag-btn">运动后</el-button>
          </el-col>
...
import {sickBloodHeighTrendDataApi} from './../../api/components/BloodheighSickcard/chart'
export default {
  name: 'chart',
  components: {
    risk,
  ...
  methods: {
    xueyaqushiOption () {
      return {
        ...
        xAxis: { // 直角坐标系grid的x轴
          ...
          boundaryGap: false,
          data: this.xueyaqushiData.label
        },
        yAxis: { // 直角坐标系grid的y轴
          type: 'value',
        // name: ''
    ...
    xueyafenbuOption () {
      return {
        ...
        legend: {
          ...
          left: 'left',
          top: 'top',
          data: this.xueyafenbulegend.status,
          selectedMode: false,
          formatter: (name) => {
          //   var index = 0
          //   this.xueyafenbulegend.status.forEach(function (value, i) {
          //     if (value === name) {
          //       index = i
          //     }
          //   })
          //   return name + '  共(' + this.xueyafenbulegend.value[index] + '次)'
          // }
            let i = 0
            this.xueyafenbuData.forEach(
              (item, index) => {
                if (item.name === name) {
                  i = index
                  return i
            ...
            return name + '  共(' + this.xueyafenbuData[i].value + '人)'
          }
    ...
    xueyaqushiGETAPI () {
      return this.$axios(sickBloodHeighTrendDataApi)
    }
  },
  computed: {
    xueyaqushiData () {
  ...
  mounted () {
    // console.log(this.xueyaqushiData)
    ...
    this.$axios.all([this.xueyaqushiGETAPI()])
    .then(this.$axios.spread((a, b, c, d) => {
      // xueyaqushi.setOption(this.xueyaqushiOption)
    }))
    .catch(err => {
      return err
    })
    xueyaqushi.setOption(this.xueyaqushiOption())
    xueyafenbu.setOption(this.xueyafenbuOption())
    xueyapingjunshuiping.setOption(this.xueyapingjunshuipingOption())
    xueyaandfood.setOption(this.xueyaandfoodOption())
    xueyaandsport.setOption(this.xueyaandsportOption())
  }
}
</script>
...
div{
  ...
  .tag-btn{
    background-color: rgb(141, 219, 167);
    width: 100%;
  }
</style>
